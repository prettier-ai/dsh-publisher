...
describe('workflows', () => {
  it('publishes dshp from both Sync and Pack CLI / Publish CLI after the CLI', () => {
    ...
    expect(sync).toContain('pnpm run --config.verify-deps-before-run=false --config.ignore-scripts=true release:publish --family vendor')
    expect(sync).toContain('pnpm run --config.verify-deps-before-run=false --config.ignore-scripts=true release:publish --family dsh')
    expect(sync).toContain("printf '%s\\n' 'verify-deps-before-run=false' 'ignore-scripts=true' >> .npmrc")
    expect(sync).toContain('scripts/install-lefthook.mjs')
    expect(sync).not.toMatch(/NPM_CONFIG_VERIFY_DEPS_BEFORE_RUN|PNPM_VERIFY_DEPS_BEFORE_RUN/)
