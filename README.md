...
- `@prettier-ai/dsh` — the CLI, with the upstream `dsh` bin. The published tarball bundles production `node_modules` and does not list the workspace `@prettier-ai/*` packages as `dependencies`, so `npm install` does not resolve that peer graph. It still ships the host-side `@deepseek-ai/*` runtime loader (plus physical `@deepseek-ai/*` symlinks inside the bundle) so existing DSH plugins keep working.
- `@prettier-ai/dshp` — thin wrapper that installs a `dshp` command and runs the same CLI as `@prettier-ai/dsh`. Its only dependency is that bundled `@prettier-ai/dsh`. Install `@prettier-ai/dshp` for a `dshp` command; `@prettier-ai/dsh` still ships `dsh`.
